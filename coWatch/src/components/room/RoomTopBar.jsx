import { Video, Users, Crown, Link2, LogOut, Copy } from "lucide-react";
    ...
    <header className="flex h-18 items-center justify-between border-b border-white/10 bg-[#12183f]/70 px-5 backdrop-blur-md">
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2 pr-4">
          ...
          <span className="text-3xl font-bold text-sky-300 md:text-2xl">CoWatch</span>
        </div>
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm text-cyan-300"
          title="Copy room ID"
        >
          ID: {roomId} <Copy size={14} />
        </button>
      </div>
